...
def create_session_factory(database_url: str = DEFAULT_DATABASE_URL, settings: Settings | None = None) -> sessionmaker[Session]:
    settings = settings or get_settings()
    engine = create_engine(database_url or settings.database_url, **_build_engine_kwargs(database_url or settings.database_url, settings))
    return sessionmaker(bind=engine, autoflush=False, autocommit=False, expire_on_commit=False, future=True)


def initialize_database(database_url: str = DEFAULT_DATABASE_URL, settings: Settings | None = None) -> None:
    settings = settings or get_settings()
    engine = get_engine_for_settings(Settings(**{**settings.model_dump(), "database_url": database_url})) if database_url != DEFAULT_DATABASE_URL else get_engine_for_settings(settings)
